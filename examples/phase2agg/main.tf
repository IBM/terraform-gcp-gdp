...
# examples/phase2agg/main.tf (GCP Version)
# Phase 2: Deploy Aggregator instances with integrated automation
...
# STEP 1: Aggregator instances (Private IP only)
...
resource "time_sleep" "guardium_boot_wait" {
  ...
  depends_on      = [google_compute_instance.aggregator]
}
...
echo ""
echo "=========================================="
echo "STEP 2: GUARDIUM AGGREGATOR AUTOMATION"
echo "=========================================="
...
MODULES_DIR="$HOME/guardium-gcp/modules/aggregator"
mkdir -p "$MODULES_DIR"
...
setup_automation_scripts() {
  ...
  for dir in "./modules/aggregator" "../modules/aggregator" "../../modules/aggregator" "/opt/guardium-gcp/modules/aggregator"; do
...
echo ""
echo "=========================================="
echo "GUARDIUM AGGREGATOR DEPLOYMENT COMPLETE"
echo "=========================================="
echo "Total VMs deployed: ${length(local.agg_config)}"
echo ""
%{for agg in local.agg_config~}
echo "VM: ${agg.vm_name} - IP: ${agg.network_interface_ip}"
%{endfor~}
echo ""
