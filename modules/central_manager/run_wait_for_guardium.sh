...
# modules/central_manager/run_wait_for_guardium.sh (GCP Version)
# Wrapper script for Guardium automation

set -eu

# Fixed log directory location
LOG_DIR="/opt/guardium-gcp/modules/central_manager/logs"
...
echo "=========================================="
echo "GUARDIUM CENTRAL MANAGER AUTOMATION LOG"
...
JSON_LOCATIONS=(
  "../../examples/phase1cm/central_manager_config.json"
  "../phase1cm/central_manager_config.json"
  "$HOME/guardium-gcp/examples/phase1cm/central_manager_config.json"
  "/opt/guardium-gcp/examples/phase1cm/central_manager_config.json"
...
if [ -z "$JSON_FILE" ]; then
  echo "[ERROR] central_manager_config.json not found in any of these locations:"
  for location in "${JSON_LOCATIONS[@]}"; do
    echo "  - $location"
  done
  exit 1
...
NEW_LOG_FILE="$LOG_DIR/cm_${PRIVATE_IP//./_}_$TIMESTAMP.log"
if [ -f "$LOG_FILE" ]; then
...
FINAL_PW=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .guardium_final_pw" "$JSON_FILE")
SUBNET_MASK=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .network_interface_mask" "$JSON_FILE")
GATEWAY=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .network_routes_defaultroute" "$JSON_FILE")
RESOLVER1=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .network_resolvers1" "$JSON_FILE")
RESOLVER2=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .network_resolvers2 // \"8.8.8.8\"" "$JSON_FILE")
HOSTNAME=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .system_hostname" "$JSON_FILE")
DOMAIN=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .system_domain" "$JSON_FILE")
TIMEZONE=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .system_clock_timezone" "$JSON_FILE")
LICENSE_KEY=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .guardium_license_key" "$JSON_FILE")
SHARED_SECRET=$(jq -r ".central_managers[] | select(.network_interface_ip == \"$PRIVATE_IP\") | .guardium_shared_secret" "$JSON_FILE")
...
if [ "$FINAL_PW" = "null" ] || [ -z "$FINAL_PW" ]; then
  ...
  jq -r '.central_managers[] | .network_interface_ip' "$JSON_FILE" | head -5
  exit 1
fi
...
if "$EXPECT_SCRIPT" "$PRIVATE_IP" "$DEFAULT_PW" "$FINAL_PW" "$PRIVATE_IP" "$DOTTED_MASK" "$GATEWAY" "$RESOLVER1" "$RESOLVER2" "$HOSTNAME" "$DOMAIN" "$TIMEZONE" "$LICENSE_KEY" "$SHARED_SECRET"; then
  echo "[$(date '+%H:%M:%S')] Guardium configuration completed successfully (GCP)"
  echo ""
  echo "==============================================="
  echo "GCP GUARDIUM CENTRAL MANAGER SETUP COMPLETE"
  echo "==============================================="
  ...
  echo "- Configured as Central Manager"
